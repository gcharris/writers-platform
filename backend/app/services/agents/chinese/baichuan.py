"""Baichuan (百川) API integration.

Baichuan is known for strong reasoning and professional writing capabilities.
"""
...
class BaichuanAgent(BaseAgent):
    """Baichuan (百川) agent for text generation.

    Supports Baichuan4 and other Baichuan models.
    """

    def __init__(self, config: AgentConfig):
        """Initialize Baichuan agent.
        ...
        self.base_url = config.base_url or "https://api.baichuan-ai.com/v1/chat/completions"

        logger.info(f"Initialized Baichuan agent with model '{self.model}'")
    ...
    async def generate(
        ...
        **kwargs
        ...
        Args:
            ...
            **kwargs: Additional Baichuan parameters
        ...
        async with httpx.AsyncClient(timeout=self.config.timeout) as client:
            ...
            if response.status_code != 200:
                raise Exception(
                    f"Baichuan API error: {response.status_code} - {response.text}"
                )
            ...
            if not choices:
                raise Exception("Baichuan API returned no choices")
