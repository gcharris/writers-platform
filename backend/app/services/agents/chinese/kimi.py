"""Kimi (月之暗面) API integration by Moonshot AI.

Kimi is known for its long context window (up to 128K tokens).
"""
...
class KimiAgent(BaseAgent):
    """Kimi (月之暗面) agent for text generation.

    Moonshot AI's model with exceptional long context capabilities.
    """
    ...
    def __init__(self, config: AgentConfig):
        ...
        logger.info(f"Initialized Kimi agent with model '{self.model}'")
    ...
    async def generate(
        ...
        **kwargs
        ...
        Args:
            ...
            **kwargs: Additional Kimi parameters
        ...
        async with httpx.AsyncClient(timeout=self.config.timeout) as client:
            ...
            if not choices:
                raise Exception("Kimi API returned no choices")
