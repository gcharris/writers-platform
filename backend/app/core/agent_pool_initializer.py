...
# ============================================================================
# Real Cloud Agents (using engine agent adapters)
# ============================================================================

# Import real agent adapters
try:
    from app.services.agents.cloud_adapters import (
        ClaudeAgentAdapter,
        GPTAgentAdapter,
        GeminiAgentAdapter,
        GrokAgentAdapter,
    ...
    logger.info("Real cloud agent adapters loaded successfully")
except ImportError as e:
    logger.warning(f"Could not import real agents, using fallback: {e}")
    REAL_AGENTS_AVAILABLE = False

    # Fallback mock agent if imports fail
    class MockCloudAgent(BaseAgent):
        """Fallback mock agent if real agents unavailable."""

        async def generate(self, prompt: str, temperature: float = 0.8,
                         max_tokens: Optional[int] = None, **kwargs) -> dict:
            mock_output = f"[Mock {self.name} response - real agents not available]"
            return {
                "output": mock_output,
                "tokens_input": 100,
                "tokens_output": 50,
...
def create_agent_pool(
    ...
    if enable_cloud and REAL_AGENTS_AVAILABLE:
        ...
        if os.getenv("ANTHROPIC_API_KEY"):
            ...
            agent = ClaudeAgentAdapter(config)
            pool.register_agent("claude-sonnet-4.5", agent, enabled=True)
            logger.info("✅ Registered Claude Sonnet 4.5 (REAL API)")
        else:
        ...
        if os.getenv("OPENAI_API_KEY"):
            ...
            agent = GPTAgentAdapter(config)
            pool.register_agent("gpt-4o", agent, enabled=True)
            logger.info("✅ Registered GPT-4o (REAL API)")
        else:
        ...
        if os.getenv("GOOGLE_API_KEY"):
            ...
            agent = GeminiAgentAdapter(config)
            pool.register_agent("gemini-2-flash", agent, enabled=True)
            logger.info("✅ Registered Gemini 2.0 Flash (REAL API)")
        else:
        ...
        if os.getenv("XAI_API_KEY"):
            ...
            agent = GrokAgentAdapter(config)
            pool.register_agent("grok-2", agent, enabled=True)
            logger.info("✅ Registered Grok 2 (REAL API)")
        else:
